...
const LOCK_INFO = [
  ...
  { key: "Voice", label: "Voice Lock — ล็อกเสียง", help: "รักษาเสียง น้ำเสียง และบุคลิกการพูดของตัวละครให้คงที่" },
...
const SERIES_TERMS = [
  ...
  ["Continuity", "ความต่อเนื่องระหว่างฉากและตอน เช่น เสื้อผ้า ตำแหน่ง อารมณ์ ของที่ถือ แสง และเรื่องราว"],
...
function createScene(index: number, duration = 6, continuity = ""): SeriesScene {
  return {
    ...
    continuityNote: continuity || "รักษาตัวตนตัวละคร เครื่องแต่งกาย เสียง Props ทิศทางการเคลื่อน แสง และสถานะเรื่องจาก Scene ก่อนหน้า",
  };
}
...
function createEpisode(number: number, previous?: EpisodeRecord): EpisodeRecord {
  ...
  return {
    ...
    continuityStart,
    endingState:"ระบุให้ชัดว่าเมื่อ Episode จบ ตัวละครแต่ละคนอยู่ที่ไหน อารมณ์อย่างไร ใครรู้อะไร มี Prop ใดอยู่กับใคร และเหตุการณ์ค้างตรงไหน",
    status:"draft",
...
const DEFAULT_SERIES: SeriesRecord = {
  ...
  locks:["Character","Voice","Visual Style","Canon","Costume"],
...
export default function SeriesManagerV3(){
  ...
  const [message,setMessage]=useState("Series Workspace พร้อมใช้งาน — เริ่มจาก Series Bible (ข้อมูลกลางของซีรีส์) แล้วทำทีละ Episode");
  ...
  function addScene(){if(remaining<1)return setMessage("เพิ่ม Scene ไม่ได้ เพราะใช้เวลาครบ Target Duration (เวลารวมเป้าหมาย) แล้ว");const next=createScene(episode.scenes.length+1,Math.min(6,remaining),scene.continuityNote);patchEpisode({scenes:[...episode.scenes,next]});setSelectedSceneId(next.id);}
  ...
  function createNext(){if(lastEpisode.status!=="completed")return setMessage(`ต้อง Complete (ทำให้เสร็จ) Episode ${String(lastEpisode.number).padStart(2,"0")} ก่อน จึงสร้างตอนถัดไปได้`);const next=createEpisode(lastEpisode.number+1,lastEpisode);setSeries(current=>({...current,updatedAt:new Date().toISOString(),episodes:[...current.episodes,next]}));setSelectedEpisodeId(next.id);setSelectedSceneId(next.scenes[0].id);setMessage(`สร้าง Episode ${String(next.number).padStart(2,"0")} แล้ว ระบบ Carry Forward (ส่งต่อ) Ending State และ Camera/Lighting จากตอนก่อนมาเป็นจุดเริ่มต้น`);}
  function markReady(){patchEpisode({status:"ready"});setMessage("Episode นี้พร้อม Production แล้ว ตรวจ Continuity Score (คะแนนความต่อเนื่อง) และ Scene Timing (เวลาของแต่ละฉาก) ก่อน Render");}
  function complete(){const now=new Date().toISOString();patchEpisode({status:"completed",updatedAt:now});try{const raw=localStorage.getItem(VIDEO_KEY);const items=raw?JSON.parse(raw) as VideoItem[]:[];const video:VideoItem={id:`video_${episode.id}`,ep:episode.number,epTitle:episode.title,projectTitle:series.title,duration:episode.duration,createdAt:now,url:`/api/mock-video?ep=${episode.number}`,status:"completed"};localStorage.setItem(VIDEO_KEY,JSON.stringify([video,...items.filter(i=>i.id!==video.id)]));window.dispatchEvent(new Event("scenova-video-library-updated"));}catch{}setMessage("Episode เสร็จสมบูรณ์ — Ending State (สถานะตอนจบ) ถูกล็อกไว้เพื่อสร้าง Episode ถัดไป และรายการส่งไป Video Library แล้ว");}
  function aiContinuity(){const previous=previousEpisode?.endingState||series.premise;patchEpisode({continuityStart:`เริ่มจาก Canon (ข้อเท็จจริงของเรื่อง) เดิม: ${previous}\nรักษา Character / Voice / Costume / Props / Location State และความสัมพันธ์เดิมทั้งหมดก่อนเพิ่มเหตุการณ์ใหม่`,endingState:`บันทึกตอนจบให้ครบ: ตำแหน่งตัวละคร, อารมณ์, ความสัมพันธ์, สิ่งที่ตัวละครรู้, Prop ที่ถืออยู่, Location, เวลาในเรื่อง และ Hook (จุดค้าง/แรงส่ง) สำหรับ Episode ${String(episode.number+1).padStart(2,"0")}`});setMessage("AI Continuity Assist (ผู้ช่วยตรวจความต่อเนื่อง) เติมโครงส่งต่อให้แล้ว กรุณาปรับรายละเอียดให้ตรงกับเนื้อเรื่องจริง");}

  return <main className={styles.main}>
    <header className={styles.hero}>
      <div><span>SCENOVA SERIES PRODUCTION</span><h1>Series Continuity Workspace — พื้นที่ทำซีรีส์ต่อเนื่อง</h1><p>สำหรับงานหลาย Episode ที่ต้องรักษาเรื่อง ตัวละคร ภาพ เสียง และ Camera Language (ภาษากล้อง) ให้ต่อเนื่อง พร้อมเครื่องมือกำกับ Scene ระดับเดียวกับ Studio</p></div>
      <div className={styles.heroActions}><Link href="/libraries?tab=videos">Video Library — คลังวิดีโอ</Link><button onClick={createNext} disabled={lastEpisode.status!=="completed"}>＋ Next Episode — ตอนถัดไป</button></div>
    </header>

    <section className={styles.guide}>
      <div className={styles.guideHead}><b>วิธีใช้งาน Series แบบง่าย</b><span>ทำตาม 4 ขั้นนี้ ระบบจะพยายามรักษา Continuity (ความต่อเนื่อง) ให้มากที่สุด</span></div>
      <div className={styles.guideGrid}>
        <article><b>1</b><strong>ตั้ง Series Bible — ข้อมูลกลางซีรีส์</strong><span>กำหนดแกนเรื่อง Character Bible, Canon Rules, Model และ Visual Style เพียงครั้งเดียว</span></article>
        <article><b>2</b><strong>ทำทีละ Episode — ทำทีละตอน</strong><span>เขียน Synopsis (สรุปเหตุการณ์) และตรวจ Continuity Start (สถานะเริ่มต้นที่รับต่อ) จากตอนก่อน</span></article>
        <article><b>3</b><strong>กำกับ Scene — กำกับแต่ละฉาก</strong><span>ปรับเวลา ระยะภาพ มุม เลนส์ การเคลื่อน แสง บทพูด และเสียงได้เหมือน Studio</span></article>
        <article><b>4</b><strong>ล็อก Ending State — สถานะตอนจบ</strong><span>ก่อนทำตอนให้เสร็จ ให้บันทึกสถานะตอนจบให้ครบ เพื่อเป็นจุดเริ่มตอนถัดไปอัตโนมัติ</span></article>
      </div>
    </section>
    ...
    <section className={styles.bible}>
      ...
      <div className={styles.four}>
        ...
        <label><span>Video Model — โมเดลวิดีโอหลัก</span><select value={series.model} onChange={e=>patchSeries({model:e.target.value})}>{MODELS.map(v=><option key={v}>{v}</option>)}</select><small>ใช้ Model เดียวกันช่วยลด Character Drift (หน้าตาหรือตัวตนตัวละครเปลี่ยนไปเอง)</small></label>
        <label><span>Visual Style — สไตล์ภาพหลัก</span><select value={series.visualStyle} onChange={e=>patchSeries({visualStyle:e.target.value})}>{STYLES.map(v=><option key={v}>{v}</option>)}</select><small>ใช้เป็น Master Style Lock (สไตล์หลักที่ทุกตอนต้องยึด)</small></label>
    ...
    <section id="episode-editor" className={styles.episodeGrid}>
      <article className={styles.card}>
        ...
        <label><span>Episode Synopsis — สรุปเหตุการณ์หลักของตอน</span><textarea value={episode.synopsis} onChange={e=>patchEpisode({synopsis:e.target.value})}/><small>ระบุ Goal (เป้าหมาย), Conflict (ความขัดแย้ง), การเปลี่ยนแปลง และสิ่งที่ต้องเกิดในตอนนี้</small></label>
      ...
      <article id="continuity" className={styles.card}>
        ...
        {previousEpisode?<div className={styles.previous}><b>Ending State — สถานะตอนจบ จาก Episode {String(previousEpisode.number).padStart(2,"0")}</b><p>{previousEpisode.endingState}</p></div>:<div className={styles.previous}><b>Episode แรก</b><p>ใช้ Series Bible เป็นจุดเริ่มต้น ไม่มี Episode ก่อนหน้า</p></div>}
        <label><span>Continuity Start — สถานะเริ่มต้นที่ต้องรับต่อ</span><textarea value={episode.continuityStart} onChange={e=>patchEpisode({continuityStart:e.target.value})}/><small>ระบบจะใช้เป็น Hard Context (ข้อมูลบังคับที่ต้องยึด) ก่อนสร้าง Scene แรก เพื่อลดเหตุการณ์ย้อนแย้ง</small></label>
        <label><span>Ending State — สถานะตอนจบสำหรับตอนถัดไป</span><textarea value={episode.endingState} onChange={e=>patchEpisode({endingState:e.target.value})}/><small>ควรระบุ Location (สถานที่), Character State (สถานะตัวละคร), Costume (ชุด), Props (ของประกอบฉาก), Emotion (อารมณ์), Story Knowledge (สิ่งที่ตัวละครรู้), Relationship (ความสัมพันธ์), Time (เวลาในเรื่อง) และ Hook (จุดค้าง/แรงส่ง)</small></label>
      </article>
    </section>

    <section id="scene-direction" className={styles.sceneSection}>
      <div className={styles.sectionTitle}><div><span>SCENE DIRECTION — การกำกับฉาก</span><h2>กล้อง ภาพ บทพูด และเสียง ต่อ Scene</h2></div><div><b>{used}/{episode.duration}s</b><span>{remaining}s remaining — เหลือ</span></div></div>
      <div className={styles.timeline}>{episode.scenes.map((s,i)=><button key={s.id} className={s.id===scene.id?styles.activeScene:""} onClick={()=>setSelectedSceneId(s.id)} style={{flexGrow:Math.max(1,s.duration)}}><b>{i+1}</b><span>{s.duration}s</span></button>)}</div>
      <div className={styles.sceneTools}><span>ทุกช่องใช้ Preset (ค่าตัวอย่างสำเร็จรูป) + Custom (กำหนดเอง) ได้เหมือน Studio</span><button onClick={addScene}>＋ Add Scene — เพิ่มฉาก</button></div>
      <div className={styles.sceneWorkspace}>
        <aside>{episode.scenes.map((s,i)=><button key={s.id} className={s.id===scene.id?styles.activeList:""} onClick={()=>setSelectedSceneId(s.id)}><b>{String(i+1).padStart(2,"0")}</b><span><strong>{s.title}</strong><small>{s.shot} • {s.lens}</small></span></button>)}</aside>
        <div className={styles.sceneEditor}>
          ...
          <div className={styles.two}><label><span>Dialogue — บทพูด</span><textarea value={scene.dialogue} onChange={e=>patchScene({dialogue:e.target.value})}/><small>ระบุ Character: Dialogue (ชื่อตัวละคร: บทพูด) และรักษาเสียง/บุคลิกตาม Character Bible</small></label><label><span>Continuity Note — สิ่งที่ Scene ถัดไปต้องรับต่อ</span><textarea value={scene.continuityNote} onChange={e=>patchScene({continuityNote:e.target.value})}/><small>บันทึก Position (ตำแหน่ง), Costume (ชุด), Prop (ของประกอบ), Emotion (อารมณ์), Camera Direction (ทิศกล้อง), Lighting (แสง) และ Story State (สถานะเรื่อง)</small></label></div>
        </div>
      </div>
    </section>
    ...
    <section className={styles.rule}><b>กติกาเพื่อ Continuity (ความต่อเนื่อง) ที่ดีที่สุด</b><span>ใช้ Series Bible เดิมตลอดงาน</span><span>ทำทีละ Episode และ Complete (ทำให้เสร็จ) ก่อนเปิดตอนถัดไป</span><span>Ending State (สถานะตอนจบ) ต้องครบก่อน Complete</span><span>Scene แรกของตอนใหม่ Carry Forward (รับช่วงต่อ) Camera/Lighting จาก Scene สุดท้ายเดิม</span><span>Character / Voice / Visual Style / Canon Lock ควรเปิดไว้เป็นค่าเริ่มต้น</span></section>
  </main>;
}
