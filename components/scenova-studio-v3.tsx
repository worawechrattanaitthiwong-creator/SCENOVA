...
const MODES = [
  {
    ...
    userControl: "แกนเรื่อง ตัวละคร โมเดล ระยะเวลา สไตล์ และแก้ค่าที่ AI เสนอได้ทุกช่อง",
  ...
  },
  {
    ...
    desc: "เหมาะสำหรับคนที่ต้องการวางจังหวะเรื่องด้วยตัวเอง ควบคุมลำดับและเวลาของแต่ละ Scene พร้อมกำหนดเป้าหมายฉาก จังหวะเรื่อง การเชื่อมฉาก และบันทึกความต่อเนื่อง",
    features: ["Scene Timeline — ไทม์ไลน์ฉาก", "Duplicate / Split / Move — จัดโครงฉาก", "Continuity — รักษาความต่อเนื่อง"],
    bestFor: "งานที่ต้องควบคุมการเล่าเรื่องเป็น Scene และต้องการจัดลำดับ/แบ่งเวลาอย่างแม่นยำ",
    userControl: "เพิ่ม ลบ ทำซ้ำ แบ่ง และย้าย Scene รวมถึง Objective, Beat, Transition และ Continuity Note",
    aiRole: "ใช้ AI Suggest เป็นตัวช่วยเฉพาะช่วงที่ต้องการ แต่โครงสร้าง Scene อยู่ภายใต้การควบคุมของผู้ใช้",
  },
  {
    id: "pro" as const,
    icon: "◆",
    ...
    desc: "เหมาะสำหรับงานที่ต้องกำหนดภาษาภาพอย่างละเอียด เช่น Focus (จุดโฟกัส), Depth of Field/DOF (ชัดตื้น-ชัดลึก), Composition (องค์ประกอบภาพ), Camera Speed (ความเร็วกล้อง), Blocking (ตำแหน่งตัวละคร), Performance (ระดับการแสดง), Color Temperature (โทนสีแสง) และ Continuity Locks (ตัวล็อกความต่อเนื่อง)",
    features: ["Technical Camera — กล้องเชิงเทคนิค", "Performance Direction — กำกับการแสดง", "Production Locks — ล็อกความต่อเนื่อง"],
    bestFor: "ผู้กำกับ/ผู้สร้างที่ต้องการควบคุมรายละเอียดภาพ การแสดง และความต่อเนื่องระดับ Shot/Scene",
    userControl: "จุดโฟกัส ชัดตื้น-ชัดลึก องค์ประกอบ ความเร็วกล้อง การแสดง Blocking โทนสี และตัวล็อกแต่ละประเภท",
    aiRole: "AI ช่วยเติมค่าได้ แต่ค่าระดับ Pro และ Lock เป็นคำสั่งหลักที่ใช้ยึดการสร้าง Prompt/Render ให้ตรงภาษาภาพที่กำหนด",
  },
];
...
export default function ScenovaStudioV3() {
  ...
  return <main className={styles.main}>
    <header className={styles.hero}><div><span>SCENOVA PRODUCTION STUDIO</span><h1>Cinematic Production Workspace</h1><p>Workflow เดียวกันทุกระดับ: Production Setup → Characters → Scene Direction → Prompt & Render แต่ความลึกของเครื่องมือจะเพิ่มขึ้นตามโหมด</p></div><div className={styles.heroActions}><span>{message}</span>{mode === "ai" ? <button className={styles.aiAction} onClick={fillProduction}>✦ AI Fill Production — ให้ AI เติมร่างทั้งงาน</button> : null}<button className={styles.primary}>Prompt & Render</button></div></header>
    ...
    <section id="scenes" className={styles.card}><div className={styles.step}><b>3</b><div><strong>Scene Direction & Camera</strong><span>ควบคุม Duration, Narrative, Camera, Lighting, Dialogue และ Continuity ต่อ Scene โดยรวมเวลาไม่เกิน Target</span></div></div>
      <div className={styles.quick}><button className={styles.aiAction} onClick={suggestSceneSection}>✦ AI ช่วยแนะนำช่วง Scene Direction & Camera</button></div>
      ...
      <div className={styles.sceneWorkspace}><aside>{scenes.map((scene,index)=><button key={scene.id} className={scene.id===selected.id?styles.sceneActive:""} onClick={()=>setSelectedId(scene.id)}><b>{String(index+1).padStart(2,"0")}</b><span><strong>{scene.title}</strong><small>{scene.duration}s • {scene.shot === "AI" ? "AI Suggest" : scene.shot}</small></span></button>)}</aside><div className={styles.sceneEditor}>
        ...
        {mode !== "ai" ? <div className={styles.field}><b>Continuity Note — บันทึกความต่อเนื่อง</b><textarea value={selected.continuity} onChange={(e)=>patchScene({continuity:e.target.value})}/><small>ระบุ Position (ตำแหน่ง), Costume (เครื่องแต่งกาย), Prop (อุปกรณ์), Emotion (อารมณ์), Lighting (แสง) และ Movement (การเคลื่อน) ที่ Scene ถัดไปต้องต่อให้ตรง</small></div>:null}
        <div className={styles.cameraGrid}><ChoiceField label="Shot Type — ระยะภาพ" help="กำหนดขนาดตัวละครหรือวัตถุหลักในกรอบภาพ" value={selected.shot} options={SHOT_TYPES} onChange={(v)=>patchScene({shot:v})}/><ChoiceField label="Camera Angle — มุมกล้อง" help="กำหนดระดับและทิศทางที่กล้องมองตัวละครหรือวัตถุ" value={selected.angle} options={CAMERA_ANGLES} onChange={(v)=>patchScene({angle:v})}/><ChoiceField label="Lens — ระยะเลนส์" help="กำหนดมุมมอง Perspective และความรู้สึกกว้าง/แคบของภาพ" value={selected.lens} options={LENSES} onChange={(v)=>patchScene({lens:v})}/><ChoiceField label="Movement — การเคลื่อนกล้อง" help="กำหนดวิธีและทิศทางที่กล้องเคลื่อนระหว่าง Shot" value={selected.movement} options={CAMERA_MOVEMENTS} onChange={(v)=>patchScene({movement:v})}/><ChoiceField label="Camera Height — ความสูงกล้อง" help="กำหนดระดับกล้องจากพื้นเพื่อควบคุมมุมมองและความรู้สึกของภาพ" value={selected.height} options={CAMERA_HEIGHTS} onChange={(v)=>patchScene({height:v})}/><ChoiceField label="Lighting — รูปแบบแสง" help="กำหนดคุณภาพ ทิศทาง และอารมณ์ของแสงใน Scene" value={selected.lighting} options={LIGHTING_STYLES} onChange={(v)=>patchScene({lighting:v})}/></div>
        <div className={styles.two}><ChoiceField label="Emotion — อารมณ์หลัก" help="กำหนดอารมณ์หลักที่ตัวละครต้องสื่อใน Scene" value={selected.emotion} options={EMOTIONS} onChange={(v)=>patchScene({emotion:v})}/><ChoiceField label="Sound / SFX / Ambience — เสียง / เอฟเฟกต์ / บรรยากาศ" help="เลือกเสียงบรรยากาศ เอฟเฟกต์ หรือพิมพ์รายละเอียดเสียงที่ต้องการเอง" value={selected.sound} options={SOUND_PRESETS} onChange={(v)=>patchScene({sound:v})}/></div>
