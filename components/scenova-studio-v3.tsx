...
const MODES = [
  { id: "ai" as const, icon: "✦", name: "AI Director", level: "AI ASSISTED", desc: "กำหนดแกนหลัก แล้วใช้ AI Suggest หรือ AI Fill Production เติม Scene, Camera, Lighting และ Dialogue ให้ทันที ทุกช่องยังเลือก Preset หรือพิมพ์ Custom เองได้", features: ["AI Fill ทั้ง Production", "AI Suggest ทุก Creative Field", "Preset + Custom ทุกช่อง"] },
  { id: "scene" as const, icon: "▦", name: "Scene Planner", level: "SCENE CONTROL", desc: "วางโครง Scene ด้วยตัวเองและใช้ AI ช่วยเฉพาะจุด มี Scene Objective, Beat, Transition, Continuity Note, Duplicate, Split และ Reorder", features: ["Scene Timeline", "Duplicate / Split / Move", "Continuity ต่อ Scene"] },
...
function ChoiceField({ label, help, value, options, onChange, ai }: { label: string; help: string; value: string; options: ProductionChoice[]; onChange: (value: string) => void; ai?: () => void }) {
  const selected = options.find((item) => item.value === value);
  return <div className={styles.field}>
    <div className={styles.fieldLabel}><b>{label}</b>{ai ? <button onClick={ai}>✦ AI Suggest</button> : null}</div>
...
function TextPreset({ label, help, value, options, onChange, ai, area = false }: { label: string; help: string; value: string; options: ProductionChoice[]; onChange: (value: string) => void; ai?: () => void; area?: boolean }) {
  return <div className={styles.field}>
    <div className={styles.fieldLabel}><b>{label}</b>{ai ? <button onClick={ai}>✦ AI Suggest</button> : null}</div>
    <select value="" onChange={(event) => event.target.value && onChange(event.target.value)}><option value="">เลือก Preset เพื่อเติมค่า...</option>{options.map((item) => <option value={item.value} key={item.value}>{item.label}</option>)}</select>
...
export default function ScenovaStudioV3() {
  ...
  function suggest(field: keyof Scene) {
    const i = Math.max(1, scenes.findIndex((scene) => scene.id === selected.id) + 1);
    const suggestions: Partial<Scene> = {
      ...
      transition: choose(TRANSITIONS, i), shot: choose(SHOT_TYPES, i + 1), angle: choose(CAMERA_ANGLES, i), lens: choose(LENSES, i + 2),
      movement: choose(CAMERA_MOVEMENTS, i + 1), height: choose(CAMERA_HEIGHTS, i), lighting: choose(LIGHTING_STYLES, i + 1), emotion: choose(EMOTIONS, i),
      dialogue: i % 2 ? "Character 1: เราคงไม่ได้บังเอิญมาเจอกันอีกใช่ไหม" : "", sound: choose(SOUND_PRESETS, i),
      focus: choose(FOCUS_OPTIONS, i), dof: choose(DOF_OPTIONS, i), composition: choose(COMPOSITION_OPTIONS, i), cameraSpeed: choose(CAMERA_SPEEDS, i),
      performance: choose(PERFORMANCE_OPTIONS, i), colorTemp: choose(COLOR_TEMPERATURES, i), blocking: "วาง Subject หลักในตำแหน่งที่สอดคล้องกับ Composition และรักษาทิศทางสายตาจาก Shot ก่อนหน้า",
    };
    patchScene({ [field]: suggestions[field] ?? selected[field] } as Partial<Scene>);
    setMessage(`AI Suggest เติม ${String(field)} ให้ Scene นี้แล้ว และยังแก้ Custom ได้`);
  }
  ...
  return <main className={styles.main}>
    ...
    <section id="setup" className={styles.card}><div className={styles.step}><b>1</b><div><strong>Production Setup</strong><span>กำหนดข้อจำกัดหลักของงาน ทุกช่องมี Preset และแก้ Custom ได้</span></div></div>
      ...
      <div className={styles.field}><div className={styles.fieldLabel}><b>Story Premise — แกนเรื่องหลัก</b>{mode === "ai" ? <button onClick={() => setStory("ตัวละครหลักพบเหตุการณ์ที่เปลี่ยนชีวิตอย่างไม่คาดคิด ความสัมพันธ์และเป้าหมายของเขาค่อย ๆ ชัดขึ้นผ่านการค้นพบในแต่ละ Scene")}>✦ AI Suggest</button> : null}</div><textarea value={story} onChange={(e) => setStory(e.target.value)} /><small>เขียนแกนเรื่องแบบสั้นหรือยาวได้ ระบบจะใช้เป็นบริบทกลางให้ Scene และ Prompt</small></div>
    </section>
    ...
    <section id="characters" className={styles.card}><div className={styles.step}><b>2</b><div><strong>Characters & Voice</strong><span>กำหนดตัวละครหลัก เลือกจาก Library หรือกรอกเอง พร้อม Voice Profile</span></div></div>
      ...
      <div className={styles.characters}>{characters.map((character) => <article key={character.id}><input className={styles.characterName} value={character.name} onChange={(e) => patchCharacter(character.id,{name:e.target.value})}/><div className={styles.two}><div className={styles.field}><b>Role — บทบาท</b><select value={character.role} onChange={(e)=>patchCharacter(character.id,{role:e.target.value})}><option>Protagonist — ตัวละครหลัก</option><option>Supporting — ตัวละครสนับสนุน</option><option>Antagonist — ฝ่ายตรงข้าม</option><option>Guest — ตัวละครรับเชิญ</option></select><small>บทบาทช่วยให้ AI ให้ความสำคัญของตัวละครถูกระดับ</small></div><div className={styles.field}><b>Voice Profile — โปรไฟล์เสียง</b><select value={character.voice} onChange={(e)=>patchCharacter(character.id,{voice:e.target.value})}>{VOICES.map(v=><option key={v}>{v}</option>)}</select><small>เลือกเสียงตัวอย่างจากคลังได้ก่อนเชื่อม Voice Provider จริง</small></div></div><div className={styles.field}><div className={styles.fieldLabel}><b>Appearance & Personality — รูปลักษณ์และบุคลิก</b>{mode === "ai" ? <button onClick={()=>patchCharacter(character.id,{appearance:"ใบหน้าจดจำง่าย รูปร่างสมส่วน การแต่งตัวสอดคล้องกับโลกของเรื่อง บุคลิกชัดเจนและรักษาความต่อเนื่องทุก Scene"})}>✦ AI Suggest</button>:null}</div><textarea value={character.appearance} onChange={(e)=>patchCharacter(character.id,{appearance:e.target.value})}/><small>ถ้ามี Reference ให้เลือกจาก Asset Library; หากไม่มีให้บรรยายผม ตา ใบหน้า รูปร่าง อายุ เสื้อผ้า และบุคลิก</small></div></article>)}</div>
    </section>
    ...
    <section id="scenes" className={styles.card}><div className={styles.step}><b>3</b><div><strong>Scene Direction & Camera</strong><span>ควบคุม Duration, Narrative, Camera, Lighting, Dialogue และ Continuity ต่อ Scene โดยรวมเวลาไม่เกิน Target</span></div></div>
      ...
      <div className={styles.sceneWorkspace}><aside>{scenes.map((scene,index)=><button key={scene.id} className={scene.id===selected.id?styles.sceneActive:""} onClick={()=>setSelectedId(scene.id)}><b>{String(index+1).padStart(2,"0")}</b><span><strong>{scene.title}</strong><small>{scene.duration}s • {scene.shot === "AI" ? "AI Suggest" : scene.shot}</small></span></button>)}</aside><div className={styles.sceneEditor}>
        ...
        <div className={styles.two}><ChoiceField label="Location — สถานที่" help="เลือก Preset หรือพิมพ์สถานที่เอง" value={selected.location} options={LOCATION_PRESETS} onChange={(v)=>patchScene({location:v})} ai={()=>suggest("location")}/><ChoiceField label="Scene Objective — เป้าหมายของ Scene" help="บอกว่า Scene นี้ต้องทำหน้าที่อะไรในเรื่อง" value={selected.objective} options={OBJECTIVE_PRESETS} onChange={(v)=>patchScene({objective:v})} ai={()=>suggest("objective")}/></div>
        <div className={styles.two}><ChoiceField label="Scene Beat — จังหวะของเรื่อง" help="เลือกตำแหน่งทาง Dramatic Structure ของ Scene" value={selected.beat} options={SCENE_BEATS} onChange={(v)=>patchScene({beat:v})} ai={()=>suggest("beat")}/><ChoiceField label="Transition — วิธีเชื่อม Scene" help="กำหนดการเปลี่ยนจาก Scene นี้ไป Scene ถัดไป" value={selected.transition} options={TRANSITIONS} onChange={(v)=>patchScene({transition:v})} ai={()=>suggest("transition")}/></div>
        <div className={styles.field}><div className={styles.fieldLabel}><b>Scene Action / Narrative — เหตุการณ์ใน Scene</b><button onClick={()=>suggest("action")}>✦ AI Suggest</button></div><textarea value={selected.action} onChange={(e)=>patchScene({action:e.target.value})}/><small>บรรยายว่าใครทำอะไร เกิดอะไรขึ้น และ Scene จบด้วยสถานะอะไร สามารถพิมพ์เองได้เต็มที่</small></div>
        ...
        <div className={styles.cameraGrid}><ChoiceField label="Shot Type — ระยะภาพ" help="กำหนดขนาด Subject ใน Frame" value={selected.shot} options={SHOT_TYPES} onChange={(v)=>patchScene({shot:v})} ai={()=>suggest("shot")}/><ChoiceField label="Camera Angle — มุมกล้อง" help="กำหนดระดับและทิศทางการมอง Subject" value={selected.angle} options={CAMERA_ANGLES} onChange={(v)=>patchScene({angle:v})} ai={()=>suggest("angle")}/><ChoiceField label="Lens — ระยะเลนส์" help="กำหนด Perspective และความรู้สึกของภาพ" value={selected.lens} options={LENSES} onChange={(v)=>patchScene({lens:v})} ai={()=>suggest("lens")}/><ChoiceField label="Movement — การเคลื่อนกล้อง" help="กำหนด Motion ของ Camera" value={selected.movement} options={CAMERA_MOVEMENTS} onChange={(v)=>patchScene({movement:v})} ai={()=>suggest("movement")}/><ChoiceField label="Camera Height — ความสูงกล้อง" help="กำหนดระดับกล้องจากพื้น" value={selected.height} options={CAMERA_HEIGHTS} onChange={(v)=>patchScene({height:v})} ai={()=>suggest("height")}/><ChoiceField label="Lighting — รูปแบบแสง" help="กำหนด Mood และคุณภาพของแสง" value={selected.lighting} options={LIGHTING_STYLES} onChange={(v)=>patchScene({lighting:v})} ai={()=>suggest("lighting")}/></div>
        <div className={styles.two}><ChoiceField label="Emotion — อารมณ์หลัก" help="อารมณ์หลักของ Subject ใน Scene" value={selected.emotion} options={EMOTIONS} onChange={(v)=>patchScene({emotion:v})} ai={()=>suggest("emotion")}/><ChoiceField label="Sound / SFX / Ambience — เสียง" help="เลือกบรรยากาศเสียงหรือพิมพ์รายละเอียดเอง" value={selected.sound} options={SOUND_PRESETS} onChange={(v)=>patchScene({sound:v})} ai={()=>suggest("sound")}/></div>
        <div className={styles.field}><div className={styles.fieldLabel}><b>Dialogue — บทพูด</b><button onClick={()=>suggest("dialogue")}>✦ AI Suggest</button></div><textarea value={selected.dialogue} onChange={(e)=>patchScene({dialogue:e.target.value})} placeholder="Character 1: ..."/><small>พิมพ์ชื่อผู้พูดและบทพูดโดยตรง หรือให้ AI Suggest แล้วแก้ต่อได้</small></div>
