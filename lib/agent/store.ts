...
function normalizeJob(row: Record<string, unknown>): AgentQueueJobRecord {
  return {
    ...
    lockedBy: row.lockedBy ? String(row.lockedBy) : null, lastError: row.lastError ? String(row.lastError) : null,
    createdAt: new Date(String(row.createdAt)), updatedAt: new Date(String(row.updatedAt)),
  };
}
...
export async function enqueueAgentStep(runId: string, payload: Record<string, unknown> = {}, delayMs = 0, maxAttempts = 3) {
  const id = randomUUID();
  const payloadJson = JSON.stringify(payload);
  const availableAt = new Date(Date.now() + Math.max(0, delayMs));
  await prisma.$executeRaw`INSERT INTO "AgentQueueJob" ("id","runId","kind","status","payloadJson","attempts","maxAttempts","availableAt","createdAt","updatedAt") VALUES (${id},${runId},'AGENT_STEP','QUEUED',${payloadJson}::jsonb,0,${maxAttempts},${availableAt},NOW(),NOW())`;
  return id;
}

export async function claimNextAgentJob(workerId: string) {
  return prisma.$transaction(async (tx) => {
    const picked = await tx.$queryRaw<Array<{ id: string }>>`SELECT "id" FROM "AgentQueueJob" WHERE "status"='QUEUED' AND "availableAt"<=NOW() ORDER BY "createdAt" ASC FOR UPDATE SKIP LOCKED LIMIT 1`;
    if (!picked[0]) return null;
    await tx.$executeRaw`UPDATE "AgentQueueJob" SET "status"='RUNNING',"lockedAt"=NOW(),"lockedBy"=${workerId},"attempts"="attempts"+1,"updatedAt"=NOW() WHERE "id"=${picked[0].id}`;
...
export async function completeAgentJob(jobId: string) {
  await prisma.$executeRaw`UPDATE "AgentQueueJob" SET "status"='COMPLETED',"lockedAt"=NULL,"lockedBy"=NULL,"updatedAt"=NOW() WHERE "id"=${jobId}`;
}
...
export async function failOrRequeueAgentJob(job: AgentQueueJobRecord, error: string, delayMs?: number) {
  if (job.attempts < job.maxAttempts) {
    ...
    await prisma.$executeRaw`UPDATE "AgentQueueJob" SET "status"='QUEUED',"availableAt"=${availableAt},"lockedAt"=NULL,"lockedBy"=NULL,"lastError"=${error},"updatedAt"=NOW() WHERE "id"=${job.id}`;
  } else {
    await prisma.$executeRaw`UPDATE "AgentQueueJob" SET "status"='FAILED',"lockedAt"=NULL,"lockedBy"=NULL,"lastError"=${error},"updatedAt"=NOW() WHERE "id"=${job.id}`;
  }
}
...
export async function getAgentRunDetails(runId: string, userId: string) {
  ...
  if (!run) return null;
  const decisions = await prisma.$queryRaw<Array<Record<string, unknown>>>`SELECT * FROM "AgentDecisionLog" WHERE "runId"=${runId} ORDER BY "createdAt" ASC`;
  const approvals = await prisma.$queryRaw<Array<Record<string, unknown>>>`SELECT * FROM "AgentApproval" WHERE "runId"=${runId} ORDER BY "requestedAt" DESC`;
  const jobs = await prisma.$queryRaw<Array<Record<string, unknown>>>`SELECT * FROM "AgentQueueJob" WHERE "runId"=${runId} ORDER BY "createdAt" DESC LIMIT 50`;
  return { run, decisions, approvals, jobs };
}
