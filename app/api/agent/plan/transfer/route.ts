...
import { issueAgentPlanTransfer } from "@/lib/agent/plan-transfer-server";
...
export async function POST(request: Request) {
  ...
  const token = issueAgentPlanTransfer({
    userId: user.id,
    target: body.data.target,
    projectKey: body.data.projectKey,
    plan: parsedPlan.data,
  });
  return NextResponse.json({ ok: true, token, expiresInSec: 900 });
}
